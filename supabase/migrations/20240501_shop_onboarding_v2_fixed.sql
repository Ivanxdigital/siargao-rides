...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'rental_shops' 
        AND policyname = 'Shop owners can update their own unverified shops'
    ) THEN
        CREATE POLICY "Shop owners can update their own unverified shops" 
        ON public.rental_shops
        FOR UPDATE
        TO authenticated
...
-- Policy for admins to manage all shops (check if it already exists)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'rental_shops' 
        AND policyname = 'Admins can manage all shops'
    ) THEN
