      ...
      {isMenuOpen && (
        ...
        >
          ...
          <div className="sticky top-0 z-50 py-4 px-6 flex justify-between items-center border-b border-white/10 bg-black/60 backdrop-blur-md">
            <div className="text-white font-semibold relative group">
              <span className="bg-clip-text text-transparent bg-gradient-to-r from-white via-white to-primary">Siargao Rides</span>
              <span className="absolute -bottom-0.5 left-0 right-0 h-px bg-gradient-to-r from-transparent via-primary/50 to-transparent"></span>
            </div>
            <button 
              onClick={toggleMenu}
          ...
          <div className="h-full overflow-y-auto pb-8">
            <motion.div 
              className="container mx-auto px-6 py-6 flex flex-col h-full"
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              ...
              {isAuthenticated ? (
                <>
                  ...
                  <div className="space-y-1">
                    ...
                    <button
                      className="flex items-center w-full py-2.5 px-3 text-white hover:text-destructive rounded-md transition-colors mt-2 text-sm"
                      onClick={() => {
                        handleLogout();
                        setIsMenuOpen(false);
                      }}
                ...
                <>
                  ...
                  <div className="flex flex-col gap-2.5 mt-2">
                    <Button asChild variant="outline" className="py-2 px-4 border-white/20 text-white hover:bg-white/10 hover:text-white w-full text-sm">
                      <Link href="/sign-in" onClick={() => setIsMenuOpen(false)}>Sign In</Link>
                    </Button>
                    <Button asChild className="py-2 px-4 bg-primary hover:bg-primary/80 w-full text-sm">
...
const MobileNavLink = ({ 
  ...
  return (
    <Link 
      href={href} 
      className="flex items-center py-2.5 px-3 text-white text-sm font-medium hover:bg-white/5 rounded-md transition-colors group"
      onClick={onClick}
    >
      <div className="flex items-center">
        {icon && <span className="mr-3 text-white/70 group-hover:text-primary transition-colors">{icon}</span>}
        <span>{children}</span>
      </div>
    </Link>
  )
